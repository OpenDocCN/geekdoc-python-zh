...
99 个练习和测验，助你学得更快、记得更牢

## 为什么选择 Python？

当你在学校开启软件工程学习之旅时，很可能会从学习 Python 开始。那么，为什么选择 Python 而不是其他编程语言呢？原因很简单：

-   无论你使用什么操作系统，Python 都易于安装。
-   Python 代码使用纯英文，易于理解。
-   Python 允许你运行有错误的代码。

这为什么是好事？
在学习过程中，你难免会犯错。Python 会在你运行代码后告诉你错误所在，帮助你找到问题。

Python 是一个非常强大的工具，但它也易于学习且上手快，这就是为什么大多数人更喜欢它的原因。

## 安装

针对不同的操作系统（如 Windows、Linux、UNIX 和 Mac OS 等），有预编译的安装包，便于简单安装。
如果你访问 [https://www.python.org/downloads/](https://www.python.org/downloads/)，可以选择适合你操作系统的安装方法。大多数 Linux 系统都预装了 Python。
本书使用的是 Python 3 版本。
在 Windows 中，安装 Python 后，可以从“开始”菜单或屏幕中选择 Python IDLE 或 Python 命令行/控制台。IDLE 和控制台的区别在于 IDLE 具有图形界面，看起来像文字处理器。在 Linux、UNIX 和 OS X 中，你可以通过在命令行输入 `python` 来启动 Python 解释器。

要选择特定版本的 Python，请输入 `pythonX`，其中 `X` 是版本号（例如，版本 3 输入 `python3`）。

## Hello World！

大多数时候，你编写 Python 代码时，会在 IDLE 或其他 IDE（如 Eclipse）或富文本编辑器（如 Sublime Text 或 Notepad++）中编写内容。但是，你也可以使用 Python 解释器以交互方式编写代码，因为它就像一个 UNIX shell。
虽然仅使用解释器来创建程序是可行的，但强烈建议不要这样做，因为很难将代码保存为文件以便重用。相反，应将解释器视为一个用于测试编码概念的“即时”工具。
现在，让我们让解释器在屏幕上输出“Hello World!”字符串。我们通过解释器来实现。

### 终端方式

使用上一章描述的 `python` 命令，从命令行启动解释器。
输入以下内容：`print("Hello world!")` 并按“Enter”键。终端将显示如下：

![](img/c1849071f36b72f8ff5736a8ef723297_4_0.png)

在上面的代码片段中，你可以看到第一行以 `>>>` 开头。
此图像显示了你向解释器提供动态输入的行。请注意，当你的第一个命令的结果打印在屏幕上后，解释器再次打印了 `>>>`。向解释器发出指令是连续的。你给出一个指令 - 等待执行完成，然后输入另一个指令，依此类推。请注意，在本书后续内容中，如果你在代码片段中看到 `>>>` 符号，这意味着你需要自己在解释器中测试该代码。
在第一行，你输入了一个语句。语句无非是给计算机执行某个任务的指令。任务可以是任何事情，从 `make_a_coffee` 到 `launch_a_rocket`。但在你的情况下，它只是一个简单的 `print` 函数。本书后面会进一步介绍函数。

### 脚本方式

计算机编程的基本原则之一是可重用性。让我们把刚才在解释器中测试过的代码变得可重用。为此，创建一个名为 `hello_world.py` 的文件，并在其中输入以下文本：

1. `print("Hello World!")`

你放入文件的内容通常就是刚才通过解释器执行的那行代码。
现在，让我们执行你编写的脚本。
打开命令行，导航到脚本所在的目录，然后输入以下命令：`python hello_world.py`。按回车键后，你会看到屏幕上出现了“Hello World!”文本。
恭喜！
...
![](img/c1849071f36b72f8ff5736a8ef723297_7_0.png)

等号用于为变量赋值。变量的名称称为其标识符。

![](img/c1849071f36b72f8ff5736a8ef723297_7_1.png)
...
Python 变量名应遵循以下规则：1. 它们必须以字母或下划线开头 2. 它们只能包含字母、数字或下划线。以下是非法变量名的几个示例：
...
![](img/c1849071f36b72f8ff5736a8ef723297_8_0.png)
...
答案：最后两个。请注意，倒数第二个语句看起来只是错误。实际上，它是一个完全合法的变量，具有合法的字符串值。

问题：在一行中为变量 `zen`、`foo` 和 `bar` 赋值 "blah"。
...
![](img/c1849071f36b72f8ff5736a8ef723297_10_0.png)
...
Python 有很多内置的算术运算符。下表简要分析了简写符号及其对应的完整形式。假设初始时以下条件成立：`a = 3`。
...
| 整除 | `a //= 2` | `a = a // 2` | 1 | 仅返回整数部分 |
| 取负 | `a = -a` | `a = 0 - a` | -3 | 返回相同值但符号相反 |
...
指数和根在Python中由标准库的函数表示，本书后续会进一步介绍。
...
下面的代码片段展示了Python解释器如何处理简单的数学运算。
...
1.  0 - 如果你的答案是其他数字，你忘记了括号内的运算应该首先执行，因为第二个括号表达式 - (4 * 4 - (2 * 6 + 4)) 的结果是0，任何数乘以0也都是0。
...
那就是错误的，因为取负（即从零减去）的优先级低于指数，因此必须用括号括起来。即使你的解决方案给出了与正确答案相同的结果，那也只是巧合。尝试在 f(x) = |x^2 - 20| 中，分别在取负运算有无括号的情况下求x的值，看看区别。

问题：如何使用Python检查一个整数是否为奇数？
回答：使用取模运算符求除以2的余数。
如果余数是1，则该数字是奇数。
...
2.  有一个字符串 value = “foobar”。你通过以下方式访问一个子字符串：
...
问题：找出这些字符串中哪个不包含字母“B”：“Foo”、“Bar”、“Zoo”、“Loo”。
答案：
...
在Python代码上写注释相当简单。有两种编写Python注释的方法：单行注释和多行注释。

注释旨在帮助用户理解代码，并被解释器忽略。单行注释通过在行首添加 # 字符来创建。它们对于在函数开头添加小注释很有用。
...
对于较大的注释，你可以在第一行和最后一行使用三个连续的引号，而不是将多个单行注释连接在一起。
...
这种注释方式通常用作任何阅读程序的人的文档。毕竟，无论你多么有才华，如果你得到一个没有任何注释的程序，除非你事先知道，否则你必须花时间研究代码以弄清楚它的作用以及如何使用它。

### 函数

我们之前谈到了可重用性是编程的一个关键原则。一旦你编写了一些代码，你会想要存储它，这样你就可以再次使用它，而无需复制和粘贴。这就是函数的作用！函数是一种包装代码的方法，这样它就可以通过调用一个简单的快捷方式再次使用。

假设你想连续三次打印“Hello Mr. Jones! How are you today?”，而没有任何代码重复。这可以通过以下方式实现：
...
在上面的代码片段中，你可以看到关键字“def”。它定义了一个处理功能的代码块。函数名需要遵循与变量名类似的规则。这个函数被命名为“hi”，后面跟着括号和冒号。括号内是参数列表，在这种情况下完全是空的。函数定义之后，代码块必须缩进以表示它是一个嵌套块。

你应该遵循两个Python惯例：使用四个空格来缩进代码块，并且在函数名中只使用小写字母和下划线。
...
在函数定义下方，你可能会看到三行调用它的代码。让我们看一个具体的调用：
...
假设你想向琼斯先生问好，但对象可以是任何名字的人。你需要将原始的`hi()`函数修改为如下形式：
...
注意，除了直接传递值，你也可以将值赋给一个变量，然后将变量传递给函数。
...
一旦你设置好函数使用参数，尝试在不提供参数的情况下调用它会导致错误。
...
假设你需要将这个逻辑封装在一个函数中，这样你就可以输入小时数和速度（km/h），然后以某种方式返回以米为单位的距离？这个逻辑可以用以下函数实现：
...
函数的最后一行包含一个`return`语句。理解它作用的最佳方法是查看该函数的几次调用：
...
正如你可能猜测的那样，`return`语句允许你将函数的结果存储在一个变量中。执行上述行之后，变量将具有以下值：
...
**问题：** 有一个数学函数 f(x,y) = x^y + 100。请在Python中实现这个函数。
...
在以下场景中，你需要知道你的车是否开得太快。如果你超过100公里/小时，函数将打印“太快了！”。如果你在限速以下行驶，函数将打印“你正在低于限速行驶。做得好。”
...
首先，`>` 运算符。它是一个布尔比较运算符，和数学中使用的一样。如果第一个值大于第二个值，它返回 `True`，否则返回 `False`。

其次，if/else 语法。如果 `is_too_fast` 为 `True`，则执行第一块代码。否则，执行第二块代码。

第三，缩进。函数内部的语句被缩进，然后 if/else 块内部的语句进一步缩进。
...
在Python中，任何值都可以被解释为 `True` 或 `False`。在数据类型中，值 `0`、`""`（空字符串）、`None` 和 `False` 本身是 `False`。任何其他值都是 `True`。

你可以通过将值转换为布尔类型来检查它是 `True` 还是 `False`：还有一些内置的比较运算符，如 `>`，在调用时返回 `True` 或 `False`。下表概述了这些运算符。
...
在许多情况下，逻辑判断可以比仅有两个条件更为精细。
让我们修改之前创建的速度函数，使其在速度超过 200 km/h 时打印“You are insane, man!!!”，并在速度介于 70 到 80 km/h 之间时打印“The optimal speed for your car.”。
...
你可能会注意到，在上面的代码中，我们并没有使用一个单独的布尔变量。相反，我们将比较运算符直接放在了 `if` 语句中。“and”运算符将两个比较运算符连接在一起。
我们还使用了关键字“elif”。这定义了一个备选分支。请注意分支的顺序！如果多个分支的条件都为真，只有第一个会被执行。例如：
...
这段代码将只会打印“Big”，而其他分支的语句都会被忽略。

#### 测验

请尝试在不使用解释器的情况下回答以下问题。

1.  以下变量的布尔值 - `bool(a)` - 分别是什么：
    1.  a = False
    2.  a = “0”
    3.  a = 0 * 18
    4.  a = “None”
...
5.  假设 x=5, y=0, 且 z=1。以下语句是 True 还是 False？
    1.  x == 0
    2.  x >= y
    3.  2 + 3 == x and y < 1
    4.  not (z == 0 or y == 0)
...
2.  False - “not”语句内部的表达式为 True，使得该部分结果为 False，而第一部分表达式也为 False。

3.  and, or, not

4.  Nickel。`x == 25` 和 `x == 10` 都为 False，所以第一个为真的表达式是 `x == 5`。

5.  a) False。运算符 `==` 用于测试相等性，而 5 不等于 0。
    b) True。5 大于或等于 0。
    c) True。`2 + 3 == x` 和 `y < 1` 两个表达式都为 True。
    d) False。`y == 0` 为 True，所以 `z == 0 or y == 0` 为 True，其否定结果为 False。
    e) True。`=` 运算符将值 3 赋给 x，而值 3 在布尔上下文中求值为 True。

6.  无输出。该条件语句为 False，因此代码块不会被执行。
...
**问题：** 编写一个语句，测试变量 x 是否在 3 到 10 之间（包含 3 和 10）。

**答案：**

x >= 3 and x <= 10

**问题：** 编写一个表达式，测试一个变量是否是偶数或是 5 的倍数。

**答案：**

(x % 2 == 0) or (x % 5 == 0)

**问题：** 有一个函数，其声明如下：`person_is_old(age)`，它根据传入的年龄（数字）返回 True 或 False。请编写一个函数，使用上面提到的函数，使得如果这个人不老，则打印“Go to work!”，否则打印“You deserved to have a long vacation”。你的函数也应该接受年龄作为其参数。
...
就像汽车驾驶员需要方向盘和油门踏板等工具来使用汽车的功能一样，你的代码的使用者需要使用称为方法的语法结构。

这里，我们用 Python 实现了一个汽车类：
...
在上面的代码中，我们创建了一个类。类是一个具有特定行为的实体。这些行为表现为方法，它们与我们之前使用过的函数类似，并且缩进方式也相同。类的方法将类本身（“self”）作为其第一个参数。“pass”关键字告诉 Python 这个代码块不执行任何操作。
这个类可以代表多种不同的汽车，比如宝马、奔驰、奥迪、菲亚特、特斯拉等等。让我们创建一个带有一些参数的汽车对象。

```python
bmw_x6 = Car(model="BMW X6", max_speed=230)
```

在上面的代码中，你可以看到汽车对象是用两个参数创建的：`model` 和 `max_speed`。最初的汽车代码中没有任何支持它们的内容。为了实现这种支持，你必须引入一个 `__init__` 方法：
...
上面的代码接收你的新汽车对象（“self”），并使用点号表示法来分配汽车的型号和最大速度。汽车的初始速度为零。`init` 方法不应包含 `return` 语句——它仅用于构造你的对象。这个方法被称为“类构造函数”。
加速和减速应该相应地增加和减少汽车的速度：
...
对象的内部状态使用点号表示法进行修改。`min` 和 `max` 函数用于确保汽车永远不会超过最高速度，也不会低于 -5 km/h（倒车档）。
以下代码将使汽车加速 30 km/h。
...
当你创建多个类时，你可能希望其中一些类之间存在关联。例如，汽车（Car）是一种交通工具（Vehicle），公交车（Bus）也是一种交通工具。尽管它们有许多相似之处，但它们的内部结构却大不相同。

为了反映对象之间的相似性和差异性，Python 有一个称为继承的特性。
...
- 1. 程序员可能会考虑哪些可供客户端使用的实践集合？
- 2. 类之间的连接叫什么？
- 3. 在Python中创建对象所用策略的总称是什么？例如，以下策略：init (name, max_speed)。
- 4. 上一个问题中的方法缺少什么？
...
问题：编写Python代码，使BMW X6加速100 km/h
...
问题：引入一个Bike类，它也是Vehicle的一种。自行车的主要特点是其速度永远不能超过30 km/h。

答案：你只需确保在Bike的构造函数中将最高速度限制为30 km/h
...
问题：在上一个练习的代码中添加一个名为 `show_status` 的新方法，该方法打印类似“这辆自行车是BMX，它的速度是27 km/h”的内容
...
在许多情况下，你需要对大量非常相似的对象执行相同的操作。例如，在现实生活中，你需要熨烫你的袜子。将每只袜子都视为需要特殊处理的独立物品，对你来说似乎并不合理。在编程中，这就是Python集合派上用场的地方。集合的一个很好的类比是一组储物柜。Python有两种类型的储物柜：一种带有非正式ID，另一种是匿名的，仅按编号列出。要访问匿名储物柜中的物品，你可以逐一查看所有储物柜，也可以通过整数索引访问特定的储物柜。在Python中，这些储物柜由列表和元组表示。对于带有非正式ID的储物柜，你可以逐一查看每个储物柜，也可以查看特定的ID。在Python中，这些储物柜由字典表示。本章概述了Python的内置集合。
...
从上面的代码片段可以看出，元组的行为与Python字符串类似——然而，字符串只是字符的集合，而元组可以包含任何类型的变量。你可以像处理字符串一样对元组进行切片、列出、连接和复制。在语法上，元组项必须用括号括起来，并用逗号分隔。即使包含单个项的元组也必须有逗号：`(1,)` 是一个元组，但 `(1)` 不是。

#### 列表

列表类似于元组，但可以在创建后进行修改。让我们定义一个包含三个项目的简单列表：
...
>>> my_list
['apple', 'pear', 'mango']
>>> my_list[2][0] = "B" # 尝试将“b”改为“B”，你会
# 得到一个错误
TypeError: 'str' object does not support item assignment
>>> new_list = ['foo', 'bar', 'zoo', 'loo']
```

正如你可能注意到的，列表项应该用方括号括起来。现在让我们删除第三个项目，并将第一个改为“blah”：
...
字典是键和值的集合，其工作方式与列表或元组非常相似。字典的值可以通过几乎任何类型的索引来寻址，而不是整数索引。让我们创建一个以人名为键、年龄为值的字典：
...
键值对必须用花括号括起来，并用逗号分隔。键和值必须用冒号分隔。字典中的键必须是唯一的，但值可以相同。键可以是几乎任何类型的变量。以下也是一个有效的字典：
...
上述语句要么更改现有键的值，要么在不存在时创建新的键值对。你也可以从字典中删除一个键值对。
...
集合可用于存储数据，但你也可以遍历它们并对每个项目单独执行操作。在Python中，你可以使用两种类型的循环：while和for。while循环只要某个条件为真就会持续重复。while循环内的语句必须缩进，就像任何其他代码块一样。循环必须使用“while”关键字、布尔语句和冒号来定义。布尔语句可以是函数调用、变量或比较语句。
...
![](img/c1849071f36b72f8ff5736a8ef723297_57_0.png)
...
#循环退出，跳过 else #返回顶部 <test1> #如果未发生 break 则运行

#### 循环

![](img/c1849071f36b72f8ff5736a8ef723297_58_0.png)
...
循环必须使用 “for” 关键字、用于保存每个单独项目的新变量名、“in” 关键字、集合名称以及冒号来定义。“for” 循环内部的语句也必须缩进。
...
2. 如何完全停止一个 while 循环？给出两种方法。 3. 如何在 for 循环中跳过特定项目？给出两种方法。

答案：

+   1. while
2. 将 while 循环的条件更改为求值为 False，或使用 break 关键字。
3. 将项目处理的代码隔离在 if 语句中，或使用 continue 关键字。

#### 练习

问题：编写一些代码来打印 ages 字典中最年长者的名字。这将用到本书前面章节的知识。
答案：
...
“open” 函数的第一个参数是文件路径，第二个参数告诉 Python 你想将数据写入文件。这意味着文件在你开始之前不必存在。该函数返回一个文件对象，你可以在之后使用。首先，让我们定义一个整数数组，并将它们作为字符串写入文件。
...
![](img/c1849071f36b72f8ff5736a8ef723297_67_0.png)

### Import 语句、Python 模块和包

正如你可能注意到的，Python 中的所有源代码都存储在称为模块的各种文件中。每个模块都有一个 .py 扩展名。不同的模块在逻辑上组合在一起，形成称为包的集合。包是一个包含 `__init__.py` 文件的普通文件夹。例如，以下是一个包：
...
虽然可以使用 Python 内置函数和方法来处理文件，但它们并不能涵盖你在操作计算机文件系统时可能遇到的所有场景。本章概述了在处理数据时可能遇到的所有常见任务，以及 Python 处理这些任务的方法。

简要说明：下面的一些示例使用了 Linux 的路径结构。若要在 Windows 上尝试，请将斜杠（/）替换为反斜杠（\）。首先，假设你有以下文件夹结构（例如，使用普通文件管理器创建）：
...
让我们验证该目录的路径是否符合预期。要获取当前目录的绝对路径，你应该使用 os.path 模块的 abspath 函数和 curdir 属性：
...
你可能已经注意到，要删除文件，必须将其名称传递给函数；要重命名文件，除了提供原始名称外，还应传递将被指定为文件新名称的字符串。注意，提供文件名时，必须给出文件的相对路径或绝对路径。对于绝对路径，一切都简单明了 - 它应该以文件系统的根目录开头。绝对路径在 UNIX 和 Windows 上可能分别如下所示：
...
相对路径是根据当前目录计算的。假设你在目录 /foo/bar/one 中，想要移动到 /foo/blah/two。在这种情况下，相对路径应为：../../two。两个点代表父目录。由于你需要向上导航两级 - 因此需要 ../../ 序列。由于 *nix 和 Windows 使用不同的分隔符，上面的 remove/rename 代码片段存在严重缺陷。它不是跨平台的。为了将其转换为跨平台代码，你应该使用 os.path 模块中的 join 函数：
...
使用 os.path 模块中的 isdir 函数，在当前目录中查找文件夹。遍历文件和文件夹，然后抛出一个自定义的 DirFound 异常，其人类可读的描述是该目录的绝对路径。
答案：
以下是一个足够的解决方案：
...
你想找出这段文本是否包含类似“dolor BLA BLA BLA amet”的内容 - 即“dolor”和“amet”按此顺序出现，中间可以有任意数量的文本。为此，你可以使用 re.search 方法：
...
“search”函数接受一个正则表达式模式作为其第一个参数，要搜索的文本作为其第二个参数。正则表达式模式是对你要在文本中查找的内容的灵活描述。格式将在本章后面介绍。
...
如果你想查看字符串是否匹配某个模式，应该使用 re.match 函数，它尝试将模式应用于整个字符串：
...
![](img/c1849071f36b72f8ff5736a8ef723297_79_0.png)

```
>>> import re
>>> print (re.search('^Beginning','Beginning of line'))
>>> print (re.search('[^line]', 'Beginning ofline')) # 不是 line
```

![](img/c1849071f36b72f8ff5736a8ef723297_79_1.png)
...
| 符号 | 描述 | 示例 |
| :--- | :--- | :--- |
| `!` | 行的开始 | `x = re.findall("^quick dog", txt)` |
| `1+` | 一个或多个 | `x = re.findall("quick+", txt)` |
| `0+` | 零个或多个 | `x = re.findall("quick*", txt)` |
| `01` | 零个或一个 | `x = re.findall("quick?", txt)` |
| `()` | 分组 | `x = re.findall("quick (a-z)+", txt)` |
| `{}` | 精确匹配 | `x = re.findall("The {2,4}", txt)` |
| `{x, y}` | 至少 x 个，至多 y 个 | `x = re.findall("The {2,4}", txt)` |
...
如你所见，一些符号以反斜杠开头。这些符号被称为转义序列，代表那些无法用其他方式描述或属于保留字符的内容。现在，让我们尝试组合符号和基数来构建模式。

一个或多个数字：`[0-9]+`
一个可选的、不以零开头的数字：`([1-9]{1}[0-9]+)?`

括号允许你将多个符号组合成符号组，并对整个组应用基数。你可以将上面提到的正则表达式重新表述为：一个可选的数字，它恰好以一个非零数字开头，后面跟着多个任意数字。
...
4.  以下调用将匹配什么：`re.search(r'..d', 'food bar')`？
尝试在不使用解释器的情况下回答。
...
def is_valid_Python_variable_name(string):
    # 下划线或字母后跟整数、
    # 下划线或字符
    found = re.match("[_A-Z][_A-Z0-9]*", string)
    return bool(found)
```
...
本章概述了 Python 标准库中的 math 模块。该模块包含常见数学运算的实现，例如平方根，这些运算未包含在标准函数集中。
使用 math 模块，你可以执行常规的三角函数运算：
...
另外，你看到的是 0.4999999999 而不是 0.5，或者 0.99999999999 而不是 1。
这是由于语言处理浮点数计算的方式所致。
...
序列化的一个现实类比是调查一个现有物品（例如一辆汽车）并为其制作详细蓝图（发动机、车轮、变速箱等）的过程。通过网络发送序列化数据类似于通过普通邮件发送蓝图。而反序列化是某人根据获取的蓝图构建原始对象副本的过程。
在 Web 编程领域，两种数据序列化标准获得了最高的普及度。XML 和 JSON。两者都是文本格式，这意味着使用这些标准表示的数据可以通过简单的文本编辑器（如 VIM）轻松地被人查看。本章的主题是 JSON 以及 Python 使用它进行数据序列化的手段。
JSON 是 JavaScript 编程语言中操作数据的原生方式。
作为一种格式，JSON 操作三种主要结构。

1.  原始类型：整数、浮点数、字符串、布尔值和 null 数据类型
2.  对象：等同于 Python 字典
3.  数组：等同于 Python 列表
...
1 如果你回答了 F - 你忘记了字符串的 JSON 表示也包括引号。
2 如果你回答了 float - 你忽略了 JSON 字符串内的双引号。
...
软件开发的主要原则之一是可复用性。这一原则可以重新表述为 DRY（不要重复自己）。实现它的最佳方式是避免重复造轮子，而是应用前人已经实现的解决方案。

在信息处理和数据存储行业数十年的发展中，出现了一套通用的约束。在此上下文中，“约束”一词指的是应用于待存储数据的一组规则。

要理解什么是约束，可以想想电子邮件地址。该地址预期包含一个“@”字符。这种预期就是一个约束。

一个具有多重约束的真实系统是汽车装配线。所有零件都必须符合特定标准，如果不符合，就无法实现完全自动化，必须引入人工操作。一个通用的经验法则是，要达到合理的自动化水平，必须牺牲一部分输入的灵活性。为了创建一个通用的约束系统，现有的系统被组合成了现在称为关系数据库的东西。

数据以表格形式存储在关系数据库中，其中每一列代表一组规则，规定每一行可以存储哪些数据类型（字符串、整数、布尔值等）。

每一列都有一个类型，该类型代表数据库引擎的一条规则，规定相应字段可以存储何种数据。类型可以是整数、字符串、布尔值等。

主键约束定义了哪个字段应被用作主索引，即行中的哪条信息可以用作其地址。“索引”一词的含义与邮政行业完全相同。它是一个唯一标识符，用于简化信息访问。在邮政系统中，它是特定收件人的地址；在关系数据库中，它是表中的特定行。

#### SQL

为了阐明引用可靠性的重要性，我们来看一个真实的例子。
...
Python 有一个内置的数据库模块叫做 sqlite3。它是关系数据库和 SQL 的完整 Python 实现。这里我们将简要介绍主要的 SQL 语句和数据库构造。
...
在上面的代码中，你可能会看到以下内容：每个用户必须有一个电子邮件和一个名字，它们都是文本值。每个用户都有一个整数标识符，用作主键。*AUTOINCREMENT* 意味着每当创建新用户时，*id* 将根据之前存储的用户标识符自动生成。
...
在上面的代码中，有一个外键定义，它表示车辆和客户之间的一种所有权关系。ON DELETE CASCADE 语句告诉数据库，每当删除一个客户时，属于该客户的所有车辆也应该被删除。
...
注意，在创建车辆时，我们为所有者传递了值 1。我们知道用户 John Smith 是数据库中第一个创建的。因此，他的 ID 合法地是 1。

让我们尝试创建一辆引用不存在用户的车，例如用户 #2。
...
```
cursor.execute('UPDATE users SET email="john.smith@newdomain.com" WHERE name="John Smith"')
```
...
在选择要检查的用户时，我们使用了 SELECT 语句。在选择过程中，我们声明我们只对用户的电子邮件感兴趣。在行选择的情况下，游标的 execute 方法返回一个数据库行的迭代器，我们将其转换为列表并打印出来。
...
最后，让我们检查 ON DELETE CASCADE 语句是如何工作的。让我们删除 John 先生。
...
```sql
DELETE FROM cars WHERE owner = 17
```
...
除了默认的Python包之外，还有各种各样的第三方解决方案。为了像使用标准库一样使用它们，它们应该被放置在PYTHONPATH下。PYTHONPATH是一个环境变量，它告诉解释器在哪里查找Python源文件。安装Python包的标准方法是使用“pip”，它有一个简单的命令行界面。这是安装方法：https://pip.pypa.io/en/latest/installing.html
...
Python在创建网站方面可能是一个极好的选择，并且有很多框架可以帮助你完成这项任务。本节概述了其中一个框架，Flask (http://flask.pocoo.org/)，它非常容易上手。Flask是一个微框架，它保持其核心简单但可扩展。基础结构被设计得非常轻量级，而新功能可以通过扩展添加。核心功能包括：
...
2.  然后，创建一个名为application的Flask类实例。
...
你可以看到从flask导入了一个request模块。这个对象代表客户端发送并由服务器接收的HTTP请求，它包含所请求页面的路径、请求类型以及关于客户端和传输数据的其他信息。在这种情况下，你使用了request对象来返回页面的路径。Flask提供了一种简单的方法来响应不同的HTTP方法：
...
到目前为止，所有页面都返回纯文本。Flask会自动将此字符串转换为HTTP响应，并为其提供mimetype“message/html”以及状态码200。现在我们应该改变这种行为。此代码返回一个JSON对象。我们还可以更改错误代码：
...
3.  5000
4.  GET、POST、HEAD、PUT和DELETE
5.  200。不要被错误名称所迷惑。你实际上从未更改过状态码。

#### 练习

问题：假设你需要为你的网站创建一个小型对话页面，允许你的用户发布和阅读消息。消息按降序排列，因此最新的消息首先显示。你的网站非常受欢迎，以至于很快就有太多的消息需要显示！你决定将每页的消息数量限制为50，并添加“下一个”和“上一个”按钮来在页面之间导航。你能编写一个脚本来显示给定页面上的第一个和最后一个帖子编号吗？
...
注意：为变量page_nb设置默认值，以便路由“/posts”和“/posts/page/1”是等效的。在装饰器中强制page_nb为int类型。这样路由“/posts/page/something”将返回404错误。

### 任务自动化配方

#### 文件和目录

处理文件和目录可能很快就会变成一项耗时且乏味的任务。Python提供了便捷的方法来帮助你轻松地遍历目录并执行文件操作。以下代码将以自顶向下方式扫描所有目录和子目录：
...
Slate是一个小模块，你可以用它轻松地从PDF文件中提取文本。以下代码读取一个PDF文件并从文档中提取所有文本，将每一页显示为一个文本字符串：
...
Python Imaging Library (PIL) 为 Python 增加了图像处理能力。你可以利用这个库来制作缩略图、在不同文档格式之间转换、打印图像等。
