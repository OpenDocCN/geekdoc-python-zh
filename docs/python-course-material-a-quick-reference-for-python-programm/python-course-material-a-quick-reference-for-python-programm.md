...
未经作者兼出版商 **Srikanth Pragada** 事先书面许可，不得以任何形式或任何方式复制或分发本书的任何部分，也不得将其存储在数据库或检索系统中，但程序可以在计算机系统中输入、存储和执行，但不得复制用于出版。

尽管在准备本书时已采取了所有预防措施，但作者对任何错误或遗漏不承担责任。对于因使用其中所含信息而造成的任何损害，也不承担任何责任。

## 关于作者

**Srikanth Pragada** 是软件培训公司 Srikanth Technologies 的总监。他于 90 年代初开始编程，曾使用过 15 种以上不同的编程语言。
...
利用图片和文字来理解概念。程序用于说明如何实现这些概念。请在您的系统中尝试本材料中给出的程序。

## 反馈请求

我们已付出相当大的努力来确保本材料内容的准确性。但是，如果您发现任何错误或有任何改进建议，请花几分钟宝贵时间发送电子邮件至 srikanthpragada@gmail.com。
...
2.  点击 Downloads 菜单并选择你的操作系统平台。
3.  这将带你到相关的下载页面。例如，对于 Windows 系统，它会跳转到 https://www.python.org/downloads/windows/。
4.  选择 Windows x86-64 可执行安装程序并下载安装文件 (python-3.10.0-amd64.exe)。
5.  运行安装程序并选择 *自定义安装*。
6.  将安装程序安装 Python 的目录更改为类似 c:\python 的路径。
7.  同时，请确保在安装窗口中选中 *Add Python 3.10 to PATH* 选项。
8.  安装程序会将所有必需的文件安装到所选文件夹中。安装程序会自动将 Python 安装文件夹添加到系统路径中。

#### 安装 Python 3.10.0 (64 位)

选择“立即安装”以使用默认设置安装 Python，或选择“自定义”以启用或禁用功能。

-   立即安装
    C:\Users\Srikanth Pragada\AppData\Local\Programs\Python\Python310
    包含 IDLE、pip 和文档
    创建快捷方式和文件关联
-   自定义安装
    选择安装位置和功能
...
-   打开命令提示符。
-   确保系统 PATH 设置为 Python 的安装文件夹。如果不是，则需要进入你安装 Python 的文件夹（例如，c:\python）。
-   运行 **python.exe** 以启动解释器。它也被称为 *读取-求值-打印循环* (REPL)。
...
-   使用 CTRL-Z 或 exit() 结束解释器并返回命令提示符。
-   解释器的行编辑功能包括交互式编辑、历史记录替换以及在支持读取行的系统上进行代码补全。

### 交互模式

-   当命令从键盘读入时，解释器处于 *交互模式*。
-   它使用 *主提示符*（通常是三个大于号 `>>>`）提示输入下一个命令；对于续行，它使用 *次提示符*（默认是三个点 `...`）。
...
> **注意：** 我们可以使用内置函数 type() 查找任何变量的数据类型。
...
> **注意：** 布尔运算符 **and** 和 **or** 被称为 *短路运算符* - 它们的参数从左到右求值，一旦结果确定，求值即停止。
...
| NotImplemented | 有一个具有此值的单个对象。此对象通过内置名称 ***NotImplemented*** 访问。数值方法和富比较方法如果未对提供的操作数实现该操作，则应返回此值。其真值为真。 |
...
| 浮点数 (float) | 这些表示机器级双精度浮点数。接受的范围和溢出处理取决于底层机器架构。Python 不支持单精度浮点数。 |
...
| 冻结集合 (frozenset) | 这些表示由内置 frozenset() 构造函数创建的不可变集合。 |
...
|---|---|
| abs(x) | 返回一个数字的绝对值。 |
| all(iterable) | 如果可迭代对象的所有元素均为真值（或者可迭代对象为空），则返回True。 |
| any(iterable) | 如果可迭代对象中任意一个元素为真值，则返回True。如果可迭代对象为空，则返回False。 |
| bin(x) | 将一个整数转换为以"0b"为前缀的二进制字符串。 |
| chr(i) | 返回一个Unicode码位为整数i的字符串表示的字符。 |
| dir([object]) | 不带参数时，返回当前本地作用域中的名称列表。带一个参数时，尝试返回该对象的有效属性列表。 |
| filter(function, iterable) | 使用那些使函数返回真值的可迭代对象元素来构建一个迭代器。 |
| format(value[, format_spec]) | 将值转换为由format_spec控制的“格式化”表示。 |
| getattr(object, name[, default]) | 返回对象object中名为name的属性值。 |
| hex(x) | 将一个整数转换为以"0x"为前缀的小写十六进制字符串。 |
| id(object) | 返回对象的“身份标识”。这是一个整数，保证在对象生命周期内唯一且恒定。 |
| len(s) | 返回对象的长度（元素个数）。 |
| `max(iterable)`<br>`max(arg1, arg2, *args)` | 返回可迭代对象中的最大项，或者返回两个或更多参数中的最大值。 |
| `input([prompt])` | 如果存在prompt参数，则将其写入标准输出，不带尾随换行符。然后该函数从输入读取一行，将其转换为一个字符串（去掉尾随的换行符），并返回该字符串。当读取到EOF时，引发EOFError。 |
| `min(iterable)`<br>`min(arg1, arg2, *args)` | 返回可迭代对象中的最小项，或者返回两个或更多参数中的最小值。 |
| `oct(x)` | 将一个整数转换为以"0o"为前缀的八进制字符串。 |
| `ord(c)` | 给定一个表示单个Unicode字符的字符串，返回一个表示该字符Unicode码位的整数。 |
| `print(*objects, sep=' ', end='\n', file=sys.stdout, flush=False)` | 将对象打印到文本流文件中，以sep分隔并在末尾添加end。如果提供了sep、end、file和flush，必须以关键字参数的形式给出。 |
| `reversed(seq)` | 返回一个反向迭代器。 |
| `round(number[, ndigits])` | 返回将number四舍五入到小数点后ndigits位精度的值。 |
| `sorted(iterable, *, key=None, reverse=False)` | 从可迭代对象的元素中返回一个新的已排序列表。 |
| `sum(iterable[, start])` | 从左到右将start与可迭代对象中的各项相加，返回总和。start默认为0。 |
...
- 可以使用`%`配合转换字符（如`%d`和`%s`）来打印格式化输出。
- 字符串的**format()**方法可用于格式化输出。调用此方法的字符串可以包含字面文本或由花括号`{}`界定的替换字段。
- 每个替换字段包含位置参数的数字索引，或关键字参数的名称。返回字符串的一个副本，其中每个替换字段都替换为对应参数的字符串值。
...
- F-string 是一个以 `f` 为前缀的字符串，当变量被花括号`{}`包围在字符串中时，会插入变量的值。
- Python 3.6 的新特性。
...
> 注意：在格式说明符中，`8.2`表示：包括小数点在内的总列数为8，小数点后有2位数字。字符`f`表示定点格式，否则使用科学计数法（E）格式。

#### IF 语句

- 用于条件执行。
- 它通过逐一评估表达式，直到找到一个为真的表达式，从而恰好选择一个代码块执行。
...
> 注意：只有当循环正常终止（即没有`break`语句）时，`while`的`else`部分才会被执行。
...
- 我们可以使用`range()`函数来生成给定起始值和结束值（不包含结束值）之间的数字。
- 如果你确实需要迭代一系列数字，内置函数`range()`会非常方便。它生成等差数列。
- 在许多方面，`range()`返回的对象的行为都像一个列表，但实际上它并不是。当你迭代它时，它会返回所需序列中的连续项，但它并不真正创建整个列表，从而节省空间。

```
range([start,] end [, step])
```

如果未给出`start`，则默认为0；如果未给出`step`，则默认为1。

```
range (10)                 # 将生成0到9
range (1, 10, 2)            # 将生成1, 3, 5, 7, 9
```

#### PASS 语句

`pass` 语句不执行任何操作。当语法上需要一条语句但程序不需要执行任何操作时，可以使用它。
...
当项目耗尽时（当序列为空或迭代器引发 `StopIteration` 异常时立即发生），如果存在 `else` 子句中的语句，则会被执行，然后循环终止。
...
![](img/6dd1124ad0340c1e75e84cf487013a5c_34_0.png)
...
![](img/6dd1124ad0340c1e75e84cf487013a5c_35_0.png)
...
>>>name[-1]       # 最后一个字符
'n'
>>>name[-3:]      # 从末尾第 3 个字符开始取
'hon'
>>>name[0:2]      # 从索引 0 到 1
'Py'
>>>name[4:]       # 从第 4 个位置开始取字符
'on'
>>> name[::-1]    # 反向取字符
'nohtyP'
>>> name[-2:-5:-1] # 从 -2 到 -5 反向取字符
'oht'
```
...
| lower() | 返回字符串的副本，所有字符转换为小写。 |
| partition(sep) | 在第一次出现 `sep` 的地方分割字符串，并返回一个三元组，包含分隔符之前的部分、分隔符本身和分隔符之后的部分。 |
| replace(old, new[, count]) | 返回字符串的副本，所有出现的子字符串 `old` 被替换为 `new`。如果给出了可选参数 `count`，则只替换前 `count` 次出现。 |
| split(sep=None, maxsplit=-1) | 返回字符串中的单词列表，使用 `sep` 作为分隔符字符串。如果给出了 `maxsplit`，则最多进行 `maxsplit` 次分割（因此，列表最多有 `maxsplit+1` 个元素）。如果未指定 `maxsplit` 或为 -1，则分割次数没有限制（进行所有可能的分割）。 |
| startswith(prefix[, start[, end]]) | 如果字符串以指定的前缀开头则返回 `True`，否则返回 `False`。 |
| strip([chars]) | 返回字符串的副本，移除开头和结尾的字符。 |
| upper() | 返回字符串的副本，所有字符转换为大写。 |
| zfill(width) | 返回字符串的副本，左侧用 ASCII '0' 数字填充，以生成长度为 `width` 的字符串。 |
...
| insert(i, x) | 在给定位置插入一个项目。第一个参数是要在其之前插入的元素的索引，因此 `a.insert(0, x)` 在列表开头插入，`a.insert(len(a), x)` 等同于 `a.append(x)`。 |
| remove(x) | 从列表中移除第一个值为 `x` 的项目。如果没有这样的项目，则会出错。 |
...
| sort(key=None, reverse=False) | 对列表中的元素进行原地排序（参数可用于自定义排序，其说明请参见 sorted()）。 |
| reverse() | 原地反转列表中的元素。 |
| copy() | 返回列表的浅拷贝。等同于 a[:]。 |
...
| s + t | s 和 t 的连接 |
| s * n 或 n * s | 等同于将 s 与其自身相加 n 次 |
| s[i] | s 的第 i 个元素，从 0 开始 |
| s[i:j] | s 从 i 到 j 的切片 |
...
- 不能为元组的单个元素赋值，但可以创建包含可变对象（如列表）的元组。
- 成员运算符 **in** 和 **not in** 可用于检查对象是否是元组的成员。
- 函数可以使用元组返回多个值。
- 空元组通过空括号构造；包含一个元素的元组通过在值后跟一个逗号来构造（仅将单个值括在括号中是不够的）。
...
- 在遍历序列时，可以使用 **enumerate()** 函数同时检索位置索引和对应的值。
...
```python
l1 = [10, 20, 30]
for i, n in enumerate(l1, start = 1):
    print(i, n)
```
...
注意：参数 **key** 用于指定一个函数，其返回值用于值的比较。稍后将介绍将函数作为参数传递。
...
| 方法 | 含义 |
|---|---|
| isdisjoint(other) | 如果集合与 other 没有公共元素，则返回 True。 |
...
-   字典通过**键**进行索引，键可以是任何不可变类型；字符串和数字通常可以作为键。
-   最好将字典理解为一组无序的 *键:值* 对，其要求是键必须唯一（在一个字典内）。
-   在花括号内放置一个以逗号分隔的 *键:值* 对列表，即可向字典中添加初始的 *键:值* 对。
-   使用不存在的键来提取值会导致错误。
-   **dict()** 构造函数可以从键值对序列直接构建字典。
...
| 方法 | 含义 |
|--------|---------|
| d[key] | 返回字典 `d` 中键为 *key* 的项。如果键不在映射中，则引发 KeyError。 |
| d[key] = value | 将 `d[key]` 设置为 value。 |
| del d[key] | 从字典 `d` 中移除 `d[key]`。如果键不在映射中，则引发 KeyError。 |
| key in d | 如果 `d` 中存在键 *key* 则返回 True，否则返回 False。 |
| key not in d | 等价于 not key in d。 |
| iter(d) | 返回一个遍历字典键的迭代器。这是 iter(d.keys()) 的快捷方式。 |
| clear() | 移除字典中的所有项。 |
| copy() | 返回字典的浅拷贝。 |
| get(key[, default]) | 如果字典中存在键 key，则返回其对应的值，否则返回 default。如果未指定 default，则默认为 None，因此此方法永远不会引发 KeyError。 |
| items() | 返回字典项（(key, value) 对）的一个新视图。 |
| keys() | 返回字典键的一个新视图。 |
| pop(key[, default]) | 如果键在字典中，移除它并返回其值，否则返回 default。如果未指定 default 且键不在字典中，则引发 KeyError。 |
| setdefault(key[, default]) | 如果键在字典中，则返回其值。否则，插入键并将值设为 default，并返回 default。*default* 默认为 None。 |
| update([other]) | 用 `other` 中的键/值对更新字典，覆盖现有的键。返回 None。 |
| values() | 返回字典值的一个新视图。 |

### 字典推导式

可以从一个可迭代对象中获取值来创建一个字典。
...
-   结构化模式匹配以 **match 语句**和带有相关操作的**模式 case 语句**的形式添加。
-   match 语句接受一个**表达式**，并将其值与作为多个 case 块给出的连续**模式**进行比较。
-   模式由序列、映射、基本数据类型以及类实例组成。
...
-   关键字 **def** 引入函数*定义*。其后必须是函数名和带括号的形式参数列表。构成函数体的语句从下一行开始，并且必须缩进。
-   函数体的第一条语句可以可选地是一个字符串字面值；这个字符串字面值是函数的文档字符串，或称为 *docstring*。
-   变量引用首先在局部符号表中查找，然后在包含函数的局部符号表中查找，接着在全局符号表中查找，最后在内置名称表中查找。
-   参数通过*按值传递*（其中*值*始终是对象*引用*，而不是对象的值）进行传递。
-   实际上，即使没有 return 语句的函数也会返回一个值——None。
...
默认值在函数定义时的*定义*作用域中求值，而不是在运行时求值。
...
```python
print_line(30, '*')  # 通过位置传递值
print_line(20)       # 使用连字符绘制一条线
print_line()         # 使用默认值绘制线
print_line(ch='*')   # 通过关键字传递值

#### 使用关键字传递值
print_line(ch='*', len=15)
```

> **注意：** 默认值只求值一次。当默认值是可变对象（如列表、字典或大多数类的实例）时，这一点尤为重要。

### 可变参数

-   函数可以通过定义带有前缀 * 的形式参数来接受任意数量的参数。
-   当函数具有可变的形式参数时，它可以接受任意数量的实际参数。
-   函数可以将可变参数与普通参数混合使用。
...
-   可以通过定义一个以 ** 为前缀的参数，将函数定义为接受任意序列的关键字参数。
-   函数将此参数视为字典，并将所有关键字参数作为字典中的键。
...
-   可以通过在参数前添加 * 来将参数定义为仅关键字参数。
-   * 之后的所有参数都必须使用关键字传递值，而不能通过位置传递。

```python
#### Parameters name & age can be passed only as keyword
#### arguments and NOT as positional
...
从 Python 3.8 开始，可以创建一个函数，使其参数只能通过位置传递，而不能通过关键字传递。

在所有需要为仅位置参数的参数后添加一个 /（斜杠）。

```python
def add(n1, n2, /):
    return n1 + n2

print(add(10, 20)) # parameters are passed by position
print(add(n1=10, n2=20)) # Can't use keyword args
```
...
函数 filter 用于从可迭代对象中选择一组元素，对于这些元素，给定的函数返回 true。给定的函数必须接受一个值并返回 true 或 false。当函数返回 true 时，该值被选中；否则，该值被忽略。
...
##### with lambda, get all odd numbers
...
names = [" PHP", " JAVA", "c", "Python ",
         " JavaScript", "C#  ", "cobol"]
#Sort after stripping spaces and converting to lowercase
...
def add_num(v, num):
    v.append(num)    # modify list

nums = []
##### Call function with a list and value to add to list
add_num(nums, 10)
add_num(nums, 20)
print(nums)
```
...
##### call swap with two int variables
a = 10
b = 20
...
-   局部函数可以从外部函数返回，然后从外部调用。
-   局部函数可以使用 **global** 关键字引用全局命名空间中的变量，使用 **nonlocal** 关键字引用外部命名空间中的变量。

```python
gv = 100  # Global variable
def f1():
    v = 200  # enclosing variable

    # Local function f2
    def f2():
        lv = 300  # local variable
        # Local, Enclosing, Global, Built-in name
        print(lv, v, gv, True)

    f2()  #call local fun from enclosing fun
...
def add_square(value):
    square = value * value # Square is local variable
    global sum            # Refer to global variable
    sum += square         # Add square to global sum

def outerfun():
    a = 10
    def innerfun():
        nonlocal a
        a = a + 1         # Increment nonlocal a
...
- 模块是一个包含 Python 定义（函数和类）以及语句的文件。
- 它可以在脚本（另一个模块）中使用，也可以在解释器的交互式实例中使用。
- 一个模块可以被*导入*到其他模块中，或者作为脚本运行。
- 文件名就是模块名加上后缀 .py。在模块内部，模块的名称（作为一个字符串）可以通过全局变量 `__name__` 获得。
- 模块可以包含可执行语句以及函数和类定义。这些语句旨在初始化模块。它们仅在 `import` 语句中*首次*遇到该模块名时执行。
...
- 系统维护一个已初始化的模块表，按模块名索引。该表可通过 **sys.modules** 访问。
- 如果没有找到匹配的文件，会引发 **ImportError**。如果找到文件，它会被解析，生成一个可执行代码块。如果发生语法错误，会引发 **SyntaxError**。
- 每当导入一个模块时，模块中的代码（而非类和函数）就会被执行。
...
它用于获取模块的成员。它返回一个已排序的字符串列表。

以下展示了 num_funs 模块的成员。
...
注意：当在模块内部使用 **dir()** 和 **__name__** 时，它们指的是当前模块。

### help() 函数

- 它调用内置的帮助系统。
- 如果没有给出参数，交互式帮助系统将在解释器控制台启动。
- 如果参数是任何其他类型的对象，将生成该对象的帮助页面。
...
当导入一个模块时，解释器首先搜索同名的内置模块。如果没有找到，则在变量 **sys.path** 给出的目录列表中搜索名为 `modulename.py` 的文件。
...
> **注意：** 由于 **sys.path** 是一个列表，因此可以向其中添加条目。使用 **r** 作为前缀表示这是一个原始字符串，这样反斜杠（\）将被视为普通字符而非特殊字符。
...
- 当将模块作为脚本运行，以及使用 `import` 语句将模块导入到另一个模块时，模块中的可执行语句都会被执行。
- 当你使用 `python filename.py` 运行一个 Python 模块时，模块中的代码将被执行，但 `__name__` 会被设置为 `__main__`。
- 但是，如果我们只想在模块作为脚本运行时执行代码，那么我们需要检查模块的名称是否被设置为 `__main__`。
...
当你将上述代码作为脚本运行 (`python.exe module1.py`) 时，会生成以下输出：
...
但是，当你将此模块导入到另一个文件（如下所示）时，则会生成以下输出。
...
- 在从命令行调用模块时，可以传递命令行参数。
- 命令行参数被放置在 `sys` 模块中的 `argv` 列表中。
...
使用 python 作为脚本运行 argv_demo.py 如下：
...
- 按照惯例，每个函数都必须使用文档字符串约定进行文档化。
- 文档字符串位于三个双引号 (""") 之间。
- 第一行应该是对象目的的简短、简洁的摘要。这一行应以大写字母开头，以句号结尾。
...
- 通常，`__init__.py` 文件用于让 Python 将该目录视为一个包；这样做是为了防止具有通用名称（例如 `string`）的目录无意中隐藏模块搜索路径中后面出现的有效模块。但是，`__init__.py` 是可选的。
...
- 为了在使用 * 导入包中的特定模块时，我们必须在包的 `__init__.py` 中定义 `__all__` 变量，列出要导入的模块。
- 如果 `__init__.py` 中未定义变量 `__all__`，那么 Python 会确保包已被导入（运行 `__init__.py` 中的初始化代码），然后导入包中定义的任何名称，但不会导入任何模块。

### stlib\_\_init\_\_.py

```python
__all__ = ["num_funs", "str_funs"]
```## PIP 与 PyPI

- PyPI（Python 包索引）是一个 Python 包的仓库。
- 网址 https://pypi.org/ 列出了所有可供下载和使用的 Python 包。
- PIP 是一个通用的 Python 包安装工具。
...
>pip uninstall requests   # 卸载 requests 包
>pip list                 # 列出所有已安装的包
```
...
- 类的*实例化*使用函数表示法。只需将类对象视为一个无参数函数，该函数返回类的一个新实例。
...
- 当一个类定义了 __init__() 方法时，类的实例化会自动为新创建的类实例调用 __init__()。
- 传递给类实例化运算符的参数会传递给 __init__()。

![](img/6dd1124ad0340c1e75e84cf487013a5c_84_0.png)
...
p = Product("Dell XPS Laptop",80000)  # 创建对象
p.print_details()
```
...
- 任何形如 __attribute（两个前导下划线）的标识符在文本上会被替换为 _classname__attribute，其中 classname 是当前类名。这个过程称为名称修饰。
- 例如，Employee 类中的 __salary 会变成 _Employee__salary。
- 但是，仍然可以从类外部访问或修改被认为是私有的变量。

| 名称    | 表示法 | 行为 |
|---------|----------|-----------|
| name    | 公有   | 可以从内部和外部访问。 |
| _name   | 受保护 | 类似于公有成员，但不应从外部直接访问。 |
...
以下代码无法访问 __name 属性，因为由于**名称修饰**，其名称被添加了类名前缀。

```
p = Product("Dell XPS Laptop", 80000)
print(p.__name)          # 将抛出错误
```
...
> 注意：对象属性 __dict__ 返回一个属性字典，其中键是属性名，值是属性值。
...
- 当类中的方法用 `@classmethod` 装饰时，它被称为类方法。
...
##### 创建一个对象
t = Time.create()    # Time 被传递给 create()
```
...
以下特殊方法表示关系运算符。通过实现这些方法，我们为用户自定义类中的这些运算符提供支持。
...
- 在 Python 中，可以使用两个装饰器 - @property 和 @setter 来创建属性。
...
- 当从现有类创建新类时，称为继承。它使我们能够在创建新类的同时重用现有类。
- 新类可以从*一个或多个*现有类创建。
...
![](img/6dd1124ad0340c1e75e84cf487013a5c_97_0.png)
...
![](img/6dd1124ad0340c1e75e84cf487013a5c_98_0.png)
...
- 子类方法被称为重写了超类中的方法。
- 重写是为了通过在子类中创建新版本来更改超类继承方法的行为。

##### 函数 isinstance() 和 issubclass()

- 函数 isinstance() 检查对象是否是类的实例。
- 函数 issubclass() 检查类是否是另一个类的子类。
...
print("Manager subclass of Employee ?? ", issubclass(Manager, Employee)) # True
```
...
Python 首先在子类中搜索属性。如果未找到，则在 superclass1 中搜索，然后（递归地）在 superclass1 的超类中搜索，如果在那里也未找到，则在 superclass2 中搜索，依此类推。
...
```Python 总是优先考虑子类版本（如果存在的话）。在下面的例子中，会调用类 C 中的 `process()` 方法，因为 Python 会优先考虑子类版本而非父类版本。因此，它不会考虑类 A 中的 `process()` 方法，因为类 A 是类 C 的父类，并且该方法在类 C 中已经存在。
...
对于上面的例子，在类 D 上调用 `mro()` 方法将返回以下结果：
...
- 抽象方法是必须由子类实现的方法。
- 当一个类中存在抽象方法时，该类必须被声明为抽象类。
- 要成为抽象类的类必须继承自 **abc** 模块的 **ABC**（抽象基类）类。
- 不能创建抽象类的实例。
...
下面的例子产生不同的结果，因为 **b** 的值为 0。我们捕获异常并在 **except** 块中使用 **ex** 来引用它。由于 **ex** 包含错误消息，打印 `ex` 将产生错误消息。**else** 块没有执行，因为 `try` 因错误而失败。
...
以下程序从用户那里获取数字，直到输入 0 为止，然后显示给定数字的总和。
...
但是这个程序很脆弱，因为任何无效输入都会导致程序崩溃，如下所示：
...
try:
    if amount < 1000:
        raise AmountError("Minimum amount is 1000")
except Exception as ex:
    print("Error : ", ex)
```
...
- 迭代器是表示数据流的对象；该对象一次返回一个数据元素。
- Python 的几种内置数据类型支持迭代，最常见的是列表和字典。
- 如果你可以为一个对象获取一个**迭代器**，那么这个对象就被称为**可迭代的**。
- 对象的 **iter()** 方法返回一个定义了 `__next__()` 方法的迭代器对象。
- `__next__()` 方法用于返回下一个元素，并在没有更多元素可返回时引发 **StopIteration** 异常。

下面的例子展示了 **list** 类如何提供 **list_iterator** 来迭代列表的元素。
...
一个**可迭代**类是提供`__iter__()`方法的类，该方法返回一个**迭代器**对象，该迭代器类提供`__next__()`方法，每次返回一个元素。

![](img/6dd1124ad0340c1e75e84cf487013a5c_114_0.png)
...
- 生成器是创建迭代器的简单而强大的工具。
- 它们像常规函数一样编写，但每当想要返回数据时，就使用**yield**语句。
- 每次对其调用**next()**时，生成器都会从上次离开的地方恢复。
- 生成器可以被视为可恢复的函数。
- 生成器可以在需要可迭代对象的地方使用。
...
> 注意：当字符串以r为前缀时，称为原始字符串。在原始字符串中，即使是像\这样的字符也被视为简单字符，而不是转义序列字符。
...
处理文件对象时，使用**with**关键字是一个好习惯。其优点是，即使在某个点引发异常，文件也会在其代码块完成后正确关闭。
...
文件对象表示一个已打开的文件。内置函数open()成功时返回文件对象。
...
| tell() | 返回文件的当前位置。 |
| seek(offset, base) | 将文件指针从给定的基点移动到所需位置。 |
...
注意：从文件读取行时，readlines()读取行时会包含行尾的换行符（\n）。

如有必要，使用str类的strip()函数来去除它。

以下程序通过读取phones.txt中的数据（如下所示），按客户姓名排序显示所有客户姓名和电话号码：
...
- 它提供诸如**dump()**用于序列化和**load()**用于反序列化的方法。
- 它使用二进制协议。

![](img/6dd1124ad0340c1e75e84cf487013a5c_122_0.png)
...
```python
contacts = (Contact("A","8888899999","a@gmail.com"),
            Contact("B","9999988888","b@yahoo.com"))
##### 使用 map() 将每个 Contact 对象转换为 dict
clist = list(map(lambda c : c.__dict__, contacts))
...
- 此模块提供对解释器使用或维护的一些变量的访问，以及与解释器紧密交互的函数。
- 它提供成员来访问命令行参数、退出程序等。
...
| path | 一个字符串列表，指定模块的搜索路径。从环境变量 PYTHONPATH 初始化，加上一个依赖于安装的默认值。 |
...
| listdir(path='.') | 返回一个列表，其中包含由 path 给出的目录中的条目名称。 |
...
##### 从给定路径获取所有文件和文件夹
allfiles = os.walk(r"c:\dev\python")
...
| 函数 | 含义 |
| --- | --- |
| group ([group1, ...]) | 返回匹配的一个或多个子组。如果只有一个参数，结果是一个字符串；如果有多个参数，结果是一个元组，每个参数对应一个项。没有参数时，group1 默认为零（返回整个匹配）。如果正则表达式使用 (?P<name>...) 语法，则 groupN 参数也可以是按组名标识组的字符串。 |
| groups() | 返回一个元组，包含匹配的所有子组，从 1 到模式中的组数。默认参数用于未参与匹配的组；默认为 None。 |
...
-   datetime 模块提供了用于以简单和复杂方式处理日期和时间的类。
-   提供了诸如 **date**、**time**、**datetime** 和 **timedelta** 等类。
-   一个 timedelta 对象表示一个持续时间，即两个日期或时间之间的差异。
-   **calendar** 和 **time** 模块提供了与日期和时间相关的其他功能。
-   **date** 类型包含年、月和日。
-   **time** 类型包含小时、分钟、秒和微秒。
-   **datetime** 类型是 date 和 time 的组合。
-   **timedelta** 类型包含天、秒、微秒。
-   它们都是不可变的。

| lastgroup | 返回最后一个匹配的捕获组的名称，如果该组没有名称或根本没有匹配任何组，则返回 None。 |
...
| date1 - date2 | 从 date1 中减去 date2 并返回 timedelta 以表示日期之间的间隔。 |
...
| 实例方法 | 含义 |
| --- | --- |
| replace (year, month, day) | 返回一个具有相同值的日期，但由关键字参数指定的参数除外。 |
| weekday() | 返回星期几的整数，其中 Monday 为 0，Sunday 为 6。 |
| isoweekday() | 返回星期几的整数，其中 Monday 为 1，Sunday 为 7。 |
| isocalendar() | 返回一个 3 元组：(ISO 年, ISO 周数, ISO 星期几)。 |
| isoformat() | 返回一个表示 ISO 8601 格式日期的字符串，格式为 'YYYY-MM-DD'。 |
| ctime() | 返回一个表示日期的字符串。 |
...
| 实例方法 | 含义 |
|---|---|
| replace(hour, minute, second, microsecond) | 返回一个具有相同值的时间，但由关键字参数指定的属性除外。 |
| isoformat (timespec='auto') | 返回一个表示 ISO 8601 格式时间的字符串，格式为 HH:MM:SS.mmmmmm，如果微秒为 0，则格式为 HH:MM:SS。 |
...
| 类方法 | 含义 |
| :--- | :--- |
| today() | 返回当前本地日期时间，tzinfo 为 None。 |
| now() | 返回当前的本地日期和时间。 |
| utcnow() | 返回当前的 UTC 日期和时间。 |
| combine (date, time) | 返回一个组合日期和时间的新 datetime 对象。 |
...
| minute | 在 range(60) 内。 |
| second | 在 range(60) 内。 |
| microsecond | 在 range(1000000) 内。 |

| 实例方法 | 含义 |
|---|---|
| date() | 返回具有相同年、月、日的 date 对象。 |
| time() | 返回具有相同小时、分钟、秒、微秒和 fold 的 time 对象。 |
| replace(year, month, day, hour, minute, second, microsecond) | 返回一个具有相同属性的 datetime，但由关键字参数指定的属性除外。 |
| weekday() | 返回星期几的整数，其中 Monday 为 0，Sunday 为 6。与 self.date().weekday() 相同。另请参见 isoweekday()。 |
| isoweekday() | 返回星期几的整数，其中 Monday 为 1，Sunday 为 7。 |
...
内部仅存储 *days*、*seconds* 和 *microseconds*。参数会被转换为这些单位：
...
| 指令 | 含义 | 示例 |
| :--- | :--- | :--- |
| %a | 星期几，根据区域设置的缩写名称。 | Sun, Mon, ..., Sat |
| %w | 星期几，以十进制数字表示，其中 0 表示星期日，6 表示星期六。 | 0, 1, ..., 6 |
| %d | 月份中的日期，以零填充的十进制数字表示。 | 01, 02, ..., 31 |
| %b | 月份，根据区域设置的缩写名称。 | Jan, Feb, ..., Dec |
| %m | 月份，以零填充的十进制数字表示。 | 01, 02, ..., 12 |
| %y | 不含世纪的年份，以零填充的十进制数字表示。 | 00, 01, ..., 99 |
| %Y | 含世纪的年份，以十进制数字表示。 | 2018 |
| %H | 小时（24小时制），以零填充的十进制数字表示。 | 00, 01, ..., 23 |
| %l | 小时（12小时制），以零填充的十进制数字表示。 | 01, 02, ..., 12 |
| %p | 区域设置中的 AM 或 PM 等效表示。 | AM, PM |
| %M | 分钟，以零填充的十进制数字表示。 | 00, 01, ..., 59 |
| %S | 秒，以零填充的十进制数字表示。 | 00, 01, ..., 59 |
| %j | 一年中的天数，以零填充的十进制数字表示。 | 001, 002, ..., 366 |
...
- 线程是一种用于解耦非顺序依赖任务的技术。
- 线程可用于提高应用程序的响应能力，使其在后台运行其他任务的同时接受用户输入。
...
| current_thread | 返回当前 Thread 对象，对应于调用者的控制线程。 |
| main_thread | 返回主 Thread 对象。在正常情况下，主线程是启动 Python 解释器的线程。 |
| enumerate | 返回当前所有存活的 Thread 对象列表。 |
...
- 它为解析后的页面创建一个解析树，可用于从 HTML 和 XML 中提取数据，这对网络爬虫很有用。
...
- 数据库 API 被称为 Python DB-API 2.0，对应 PEP 249，详见 https://www.python.org/dev/peps/pep-0249
- 它使得代码通常具有更好的跨数据库可移植性，因为所有数据库模块都提供相同的 API。
- 访问数据库所需的模块需要下载。
- 用于访问 SQLite 数据库的模块 sqlite3 随 Python 一起提供。

![](img/6dd1124ad0340c1e75e84cf487013a5c_152_0.png)## SQLite3 数据库

-   SQLite 是一个 C 语言库，它提供了一个轻量级的基于磁盘的数据库，无需单独的服务器进程，并允许使用非标准的 SQL 查询语言变体来访问数据库。
-   可以使用 SQLite 来原型化一个应用程序，然后将代码移植到像 Oracle 这样的大型数据库上。
-   Python 自带了 SQLite 数据库。
...
| :--- | :--- | :--- |
| NULL | 值为 NULL。 | None |
| INTEGER | 值为有符号整数，根据值的大小存储为 1、2、3、4、6 或 8 字节。 | int |
| REAL | 值为浮点值，存储为 8 字节 IEEE 浮点数。 | float |
| TEXT | 值为文本字符串，使用数据库编码（UTF-8、UTF-16BE 或 UTF-16LE）存储。 | str |
| BLOB | 值为数据块，完全按照输入时的样子存储。 | bytes |
...
|---|---|
| close() | 关闭连接。 |
| commit() | 将事务中挂起的更改提交到数据库。 |
| rollback() | 使数据库回滚到任何挂起事务的开始。在未先提交更改的情况下关闭连接将导致执行隐式回滚。 |
| cursor() | 使用此连接返回一个游标对象。 |

### 游标对象

-   游标代表一个数据库游标，用于管理获取操作的上下文。
-   从同一连接创建的游标不是隔离的，即，一个游标对数据库所做的任何更改都会立即对其他游标可见。

| 方法 | 含义 |
|--------|---------|
| close() | 关闭游标。 |
| execute(operation [, parameters]) | 准备并执行一个数据库操作。 |
| executemany(operation, seq_of_parameters) | 准备一个数据库操作（查询或命令），然后对 `seq_of_parameters` 序列中找到的所有参数序列或映射执行该操作。 |
| fetchone() | 获取查询结果集的下一行，返回一个单独的序列，或者在没有更多数据可用时返回 None。 |
| fetchmany([size=cursor.arraysize]) | 获取查询结果的下一批行，返回一个序列的序列（例如，元组列表）。当没有更多行可用时，返回一个空序列。 |
| fetchall() | 获取查询结果的所有（剩余）行，将它们作为序列的序列返回（例如，元组列表）。请注意，游标的 `arraysize` 属性可能会影响此操作的性能。 |

| 属性 | 含义 |
| --- | --- |
| rowcount | 此只读属性指定最后一次 `execute()` 方法检索或影响的行数。 |
| lastrowid | 只读属性，提供最后修改行的 rowid。 |
| arraysize | 读/写属性，控制 `fetchmany()` 返回的行数。默认值为 1，这意味着每次调用将获取单行。 |
| connection | 此只读属性提供 Cursor 对象使用的 SQLite 数据库连接。 |

**注意：** 使用 SQLite Studio（一个免费的 GUI 工具）来管理 SQLite 数据库。可以从 [https://sqlitestudio.pl/index.rvt](https://sqlitestudio.pl/index.rvt) 下载它。
...
以下程序展示了如何从用户获取数据，并将其插入到 `EXPENSES` 表中。
