...
本书是Python技巧和窍门的集合。我整理了100个Python技巧和窍门，如果你正在学习Python，可能会发现它们很有帮助。这些技巧主要涵盖Python的基础和中级水平。

在本书中，你将找到关于以下内容的技巧和窍门：
...
为了充分受益，请尽力将代码写下来并运行。重要的是你要尝试理解代码是如何工作的。修改并改进代码。不要害怕尝试。

最终，我希望这些技巧和窍门能帮助你提升Python技能和知识。

让我们开始吧。

## 1 | 水平打印

当遍历一个可迭代对象时，print函数会将每个项目打印在新行上。这是因为print函数有一个名为end的参数。默认情况下，end参数有一个换行符（end = "\n"）。现在，要水平打印，我们需要移除换行符并用空字符串替换它（end = ""）。在下面的代码中，请注意逗号之间的空格（" "）；这是为了确保数字之间用空格打印。如果我们移除空格（""），数字将这样打印：1367。以下是演示代码：
...
Python有一个名为**calendar**的内置模块。我们可以导入此模块来打印日历。我们可以用日历做很多事情。
...
你可以用日历做很多其他事情。例如，你可以用它来检查给定的年份是否是闰年。让我们检查2022年是否是闰年。
...
以下代码演示了如何使用*datetime()*模块获取当前时间。*strftime()*方法可以格式化时间以获得所需的输出。此代码分解了如何使用datetime模块和*strftime()*方法来获取小时、分钟和秒格式的格式化时间字符串。
...
*sort()*方法将按升序（默认）对列表进行排序。要使sort方法正常工作，列表中的对象应具有相同的数据类型。你不能对混合了不同数据类型（如整数和字符串）的列表进行排序。*sort()*方法有一个名为reverse的参数；要按降序对列表进行排序，请将reverse设置为True。
...
在Python中，一旦变量被赋值给对象，就可以交换它们。下面，我们最初将20赋给$x$，将30赋给$y$，但随后我们交换了它们：$x$变成30，$y$变成20。
...
在Python中，我们也可以使用XOR（异或）运算符来交换变量。这是一个三步方法。在下面的例子中，我们正在交换$x$和$y$的值。
...
如果你想知道一个项目在可迭代对象中出现了多少次，可以使用collection模块中的*counter()*类。*counter()*类将返回一个字典，显示每个项目在可迭代对象中出现的次数。假设我们想知道名字Peter在以下列表中出现了多少次；以下是我们如何使用collections模块的*counter()*类。
...
假设你有一个负数，你想返回该数的绝对值；你可以使用 `abs()` 函数。Python 的 `abs()` 函数用于返回任何数字（正数、负数和复数）的绝对值。下面，我们演示如何从一个包含正数和负数的数字列表中返回一个绝对值列表。我们使用列表推导式。
...
你注意到我们在两种情况下都使用了列表推导式来添加分隔符吗？很酷的事情，对吧？😊。
...
我们也可以使用 *sorted()* 函数来检查两个字符串是否是变位词。默认情况下，*sorted()* 函数会按升序对给定的字符串进行排序。因此，当我们传递字符串作为参数给 sorted 函数进行相等性测试时，首先对字符串进行排序，然后进行比较。见下面的代码：
...
要使用 Python 打开网站，需要导入 *webbrowser* 模块。这是一个内置模块，所以你无需安装任何东西。下面，我们尝试使用该模块的 *open()* 方法打开 google.com。如果你知道任何网站的 URL，都可以这样做。
...
假设你有一个字符串，并想找出字符串中出现频率最高的元素；你可以使用 `max()` 函数。`max()` 函数会统计字符串中的项目，并返回出现次数最多的项目。你只需将字符串的 `count` 方法传递给 key 参数即可。让我们用下面的字符串来演示这一点。
...
我们也可以使用 collections 模块中的 **Counter** 类。这个类的 `most_common()` 方法会统计列表中每个元素出现的次数，并返回所有元素及其计数，以元组列表的形式返回。下面，我们传递参数 (1)，因为我们希望它返回列表中出现次数排名第一的元素。如果我们传递 (2)，它将返回出现次数最多的两个元素。
...
*sys* 模块有一个方法可以用于此类任务。下面是一段演示代码。给定相同的项目，集合、元组和列表中哪一个内存效率更高？让我们使用 sys 模块来找出答案。
...
你可以使用 operator 模块的 *itemgetter()* 类对元组列表进行排序。*itemgetter()* 函数作为 key 传递给 *sorted()* 函数。如果你想按名字排序，就将索引 (0) 传递给 *itemgetter()* 函数。下面是你使用 *itemgetter()* 对元组列表进行排序的不同方式。
...
由于 `enumerate` 会对其遍历的项目进行计数（添加一个计数器），你可以用它来创建一个元组列表。下面，我们从一个日期列表中创建一个包含一周各天的元组列表。`enumerate` 有一个名为 `start` 的参数。`start` 是你希望计数开始的索引。默认情况下，`start` 为零（0）。
...
也可以使用 *for 循环* 创建相同的输出。让我们创建一个函数来演示这一点。
...
下面，我们在代码中插入一个 `assert` 语句。这段代码接收一个名称列表，并返回所有小写字母的名称。我们期望列表中的所有项目都是字符串，因此我们使用 `insert` 语句来调试非字符串条目。`insert` 语句将检查列表中的所有项目是否都是 *str* 类型。如果其中一个项目不是字符串，它将评估为 `False`。它将停止程序并抛出一个 *AssertionError*。它将显示错误消息 "**non-string items are in the list.**"。如果所有项目都是字符串，它将评估为 `True` 并运行其余代码。由于名称列表中的第四个名称不是字符串，代码返回错误。
...
如果我们想打印字符串中的所有元素及其索引，可以这样使用 `enumerate`。
...
什么是嵌套列表？嵌套列表是指一个列表的元素是另一个列表（即列表中的列表）。你可以使用 `sum()` 函数来展平嵌套列表。请注意，这适用于二维嵌套列表。
...
请注意，这不是展平列表最高效的方法，可读性也不强。但知道这个技巧仍然很酷，对吧？😉

### 使用 reduce 函数

这里还有另一个酷炫的技巧，可以用来展平二维列表。这次我们使用 `functools` 模块中的 `reduce`。这是 Python 中的另一个高阶函数。
...
在下面的代码中，我们使用文件名作为参数，因为我们假设文件与 Python 文件在同一文件夹中。从输出中你可以看到，即使文件不存在，我们的代码也不会产生错误。
...
集合推导式与列表推导式类似；唯一的区别是它返回的是一个集合而不是一个列表。集合推导式使用花括号而不是方括号。这是因为集合是用花括号括起来的。你可以对可迭代对象（列表、元组、集合等）使用集合推导式。
...
当你不确定函数需要多少个参数时，你可以将 *args（非关键字参数）作为参数传递。* 符号告诉 Python 你不确定需要多少个参数，Python 允许你传入任意数量的参数。下面，我们用不同数量的参数计算平均值。首先，我们传递三个（3）个数字作为参数。然后我们传递六个数字作为参数。*args 使函数在参数方面更加灵活。
...
假设我们有一个名字列表，我们想返回一个小写名字的列表；以下是你可以使用 *filter()* 函数实现的方法。
...
下面，我们有一个汽车品牌列表和一个汽车型号列表。我们将创建一个 DataFrame。DataFrame 将有一个名为 **Brands** 的列，另一个名为 **Models**，**索引**将是升序的数字。
...
让我们继续上一个技巧（38）的内容。pandas DataFrame 最重要的特性之一是其极高的灵活性。我们可以添加和删除列。现在，让我们向 DataFrame 添加一个名为 Age 的列，该列将包含汽车的年龄信息。
...
Pandas 提供了一个 *drop()* 方法，可用于从 DataFrame 中删除列和行。假设我们想从上面的 DataFrame 中删除 "Models" 列。从输出中可以看到，"Models" 列已被移除。*inplace=True* 表示我们希望更改直接作用于原始 DataFrame。在 DataFrame 中，axis 1 代表列。因此，当我们将 1 传递给 axis 参数时，我们就是在删除一列。
...
下面，我创建了一个计时器函数，它使用了 time 模块的 *perf_counter* 类。请注意，*inner()* 函数位于 *timer()* 函数内部；这是因为我们正在创建一个装饰器。*inner()* 函数内部是“被装饰”函数运行的地方。基本上，“被装饰”函数作为参数传递给装饰器。然后，装饰器在 *inner()* 函数内部运行这个函数。

*range_tracker* 函数前的 @timer 表示该函数正被另一个函数装饰。**“装饰”一个函数意味着在不改变该函数本身的情况下，为其改进或添加额外功能。** 通过使用装饰器，我们能够为 *range_tracker* 函数添加一个计时器。我们使用这个计时器来检查从 range 创建一个列表需要多长时间。
...
生成器类似于列表推导式，但使用的是圆括号而不是方括号。生成器一次生成一个元素，而列表推导式则一次性释放所有元素。下面，我从两个方面对列表推导式和生成器进行比较：
...
列表推导式执行速度快得多，但占用更多内存。生成器执行稍慢，但由于它们一次只生成一个元素，因此占用的内存更少。
...
假设你有一个名字列表，想将它们写入一个文件，并且希望所有名字都垂直排列。下面是一个示例代码，展示了如何实现。以下代码创建了一个 CSV 文件。我们通过使用转义字符（'\n'）告诉代码将每个名字写在新的一行上。另一种创建 CSV 文件的方法是使用 CSV 模块。
...
要使此代码成功运行，请确保要合并的文件与你的 Python 文件保存在同一位置。创建一个包含所有要合并文件的列表。输出仅确认合并文件已创建。该文件的名称是 *mergedfile.pdf*。
...
Python 之禅是编写优美代码的 19 条指导原则列表。Python 之禅由 Tim Peters 撰写，后来被添加到 Python 中。
...
你知道吗？你可以使用 pprint 模块打印出排序后的字典。下面，我们使用该模块按升序打印一个字典。字典按键排序。
...
当你想使用 CV2 查看图像时，会打开一个窗口。*waitkey* 表示我们期望显示窗口保持打开的时间。如果在显示时间结束前按下了某个键，窗口将被销毁或关闭。
...
如果你想了解给定代码的执行时间，可以使用 *timeit()* 模块。下面，我们创建一个名为 `timer` 的函数，该函数使用 *timeit* 模块中的 *timeit()*。我们基本上是使用 **timeit** 来确定运行 **sum(num**2 for num in range(10000))** 需要多长时间。下面代码中 timeit 函数的第一个参数是 *stmt*。这是我们传递想要测量的 Python 代码的地方。此参数只接受字符串作为参数，因此我们必须确保将代码转换为字符串格式。*number* 参数基本上是我们希望在此代码上运行的执行次数。
...
不一定需要像上面那样创建一个函数。你也可以在不创建函数的情况下使用 *timeit*。让我们通过不使用函数来简化上面的代码。请记住，*stmt* 参数只接受字符串作为参数；这就是为什么下面的 `test` 变量（我们传递给 *timeit()* 函数的代码）是一个字符串。
...
安装后，你可以将其导入到你的脚本中。下面的函数演示了我们如何使用 pyshorteners。我们向函数传递一个非常长的 URL，它返回一个短 URL。
...
"G2AArM2tCrJfyfVcqq6z8Rwg%3A1662044120968&ei=2McQY4PaOp68xc8Pyp-qIA&qq=python+do&s_gs_lcp="
"Cgdnd3Mtd2l6EAEYATIFCAAQgAQyBQgAEIAEMgUIABCABDIFCAAQgAQyBQgAEIAEMgUIABCABDIFCAAQ"
...
Python 有一个名为 `pdf2docx` 的库。借助这个库，你只需几行代码就能将 **pdf 文件**转换为 **word** 文档。使用 pip 安装该库：

**pip install pdf2docx**

我们将从这个模块导入 `Converter` 类。如果文件与你的 Python 脚本在同一目录下，那么你只需提供文件名而不是链接。新的 doc 文件将保存在同一目录中。
...
我们可以使用 Python 库 **PyPDF2** 从 PDF 中提取文本。首先，使用 pip 安装该库：

**pip install PyPDF2**

我们使用模块中的 **PdfFileReader** 类。这个类将返回 PDF 文档中的文件数量，并且它有一个 `get page` 方法，我们可以用它来指定要从中提取信息的页面。下面，我们从书籍 [50 Days of Python](https://www.google.com) 中提取文本。
...
输出：
Day 2 : 字符串转整数
编写一个名为 `convert_add` 的函数，它接受一个字符串列表作为参数，将其转换为整数并求和。例如，`['1', '3', '5']` 应转换为 `[1, 3, 5]` 并求和为 9。
...
你是否曾好奇过你安装的库在你的机器上的位置？Python 有一种非常简单的语法来获取已安装库的位置。如果你安装了 pandas，下面是如何找到它的位置。这是我在我的电脑上运行它时得到的结果：
...
用 Python 创建条形码怎么样？你可以使用 `python-barcode` 来生成不同类型的对象。首先，安装条形码库：**pip install python-barcode**
...
![](img/18cae09c3dc47caa2996fe5aa9c10ef1_69_0.png)
...
如果我们只想返回列表中某个名字的索引，我们可以将 `len()` 和 `range()` 函数与条件语句结合起来。假设我们想要名字 "Messi" 的索引；以下是我们可以如何做到：
...
你知道吗？你可以从表情符号中提取文本。假设你有一段带有表情符号的文本，但你不知道这些表情符号是什么意思。你可以使用一个 Python 库将表情符号转换为文本。
...
![](img/18cae09c3dc47caa2996fe5aa9c10ef1_73_0.png)
...
你可以使用一个名为 **langdetect** 的 Python 库来检测文本中的语言。目前，该检测器支持大约 55 种语言。以下是支持的语言：

![](img/18cae09c3dc47caa2996fe5aa9c10ef1_74_0.png)
...
你知道吗？你可以使用 Python 来刷新一个 URL。通常，要刷新页面，我们必须手动操作。然而，我们只需几行代码就能实现这个过程的自动化。为此，我们将使用 **selenium** 模块。请安装以下内容：

**pip install selenium**
**pip install webdriver-manager**

现在，让我们来编写代码。我使用的是 Chrome 网页浏览器，因此需要 Chrome 的依赖项。如果你使用其他浏览器，则需要安装该浏览器的驱动程序。所以，我们需要想要打开的网站的 URL 链接。时间是我们希望在刷新页面前等待的秒数。一旦等待时间结束，代码将自动刷新页面。
...
如果你想检查一个字符串是否是另一个字符串的子串，可以使用 **in** 和 **not in** 运算符。假设我们想测试 "like" 是否是字符串 **s** 的子串。以下是我们使用 **in** 运算符的方法：如果 "like" 是 s 的子串，**in** 运算符将求值为 **True**，否则为 **False**。
...
Python 建议仅使用 **find()** 方法来获取子串的位置。如果我们想找到 "something" 在字符串中的位置，可以这样使用 **find()** 方法：find 方法返回子串开始的索引。
...
如果你有两个列表，并且想找出它们的差集，即存在于 **列表 a** 中但不在 **列表 b** 中的元素，请使用 **set().difference(set())** 方法。
...
如果你有一个字典列表，并且想根据它们的值进行排序，可以使用 operator 模块中的 *itemgetter* 类和 sorted 函数。下面是一个示例。
...
上面的示例按**升序**对列表进行排序。如果你想按**降序**对列表进行排序，则需要将 sorted 函数中的 reverse 参数设置为 True。你可以看到下面的列表已按降序排序。
...
如果你想从用户那里获取多个输入，该怎么办？在 Python 中如何实现？Python 使用 *input()* 函数从用户那里获取输入。*input()* 函数一次只接受一个输入；这是默认设置。我们如何修改 input 函数使其一次接受多个输入呢？我们可以将 *input()* 函数与字符串方法 *split()* 一起使用。语法如下：

```python
input().split()
```

借助 *split()* 函数，*input()* 函数可以同时接受多个输入。*split()* 方法将输入分隔开。默认情况下，split 方法在空白处进行分隔。但是，你也可以指定一个分隔符。下面，我们要求用户输入三个（3）个数字。我们使用 *split()* 方法指定分隔符。在这种情况下，我们用逗号（,）分隔输入。然后我们计算这三个数字的平均值。如果输入是 12、13 和 14，我们将得到以下输出：
...
我们也可以从用户那里获取多个值。下面，我们使用列表推导式要求用户输入多个名字。如果用户输入 Kelly、John、Trey 和 Steven，输出将是：
...
如果你想为一个对象创建一个迭代器，请使用 *iter()* 函数。一旦迭代器对象被创建，就可以逐个访问元素。要访问迭代器中的元素，你需要使用 *next()* 函数。下面是一个示例：
...
为什么要使用迭代器？迭代器具有更好的内存效率。

## 69 | 将两个列表合并为字典

有时，在处理列表时，你可能想将数据类型更改为字典。也就是说，你可能想将列表合并成一个字典。为此，你可能需要使用 `zip()` 函数和 `dict()` 函数。`zip()` 函数接受两个可迭代对象并将元素配对。第一个可迭代对象中的第一个元素与第二个可迭代对象中的第一个元素配对，第二个元素与另一个第二个元素配对，依此类推。`zip()` 函数返回一个元组的迭代器。`dict()` 函数会将配对的元素转换为键值组合，从而创建一个字典。
...
字符串的排列是指我们可以排列字符串元素的不同顺序。例如，给定一个 "ABC" 字符串，我们可以将其重新排列为 ["ABC", "ACB", "BAC", "BCA", "CAB", "CBA"]。在 Python 中，查找字符串排列最简单的方法是使用 *itertools*。Itertools 有一个排列类。以下是如何使用 itertools 来实现。
...
有时你想解包一个列表，并将元素分配给不同的变量。Python 有一种方法可以让你的生活更轻松。我们可以使用 Python 的解包运算符，即星号 (*)。下面是一个名字列表。我们想获取男孩的名字并将其赋值给变量 "boy_name"。其余的名字将赋值给变量 "girls_name"。因此，我们将列表中的第一项赋值（从列表中解包）给 boy 变量。然后我们在 **girls_name** 变量前加上 "*"。通过在女孩名字前加上 *，我们基本上是在告诉 Python，一旦它解包了男性名字，所有其他剩余的名字都必须赋值给 **girls_name** 变量。请看下面的输出：
...
Python 提供了一种方式来提示函数中期望的参数类型或函数应返回的数据类型。这些被称为 "类型提示"。以下是类型提示使用的一个示例。
...
这里我们有一个简单的函数。注意函数有一个参数 **s**。参数 s 后面的 "str" 是一个类型提示。它简单地表示该函数期望（或提示）传递给参数 "s" 的参数必须是一个字符串。注意到末尾的 "-> bool" 了吗？这是另一个类型提示。它提示该函数期望的返回值是一个布尔值。现在，Python 在运行时并不强制执行提示；这意味着如果传递了一个非字符串参数并且函数返回了一个非布尔值，代码仍然可以正常运行。

为什么需要类型提示？类型提示只是帮助使代码更具可读性和描述性。仅通过查看提示注释，开发人员就能知道函数的期望是什么。

## 73 | 文件位置

你想知道你的 Python 文件所在的目录吗？使用 **os** 模块。**os** 模块有一个 **getcwd()** 方法，它返回目录的位置。只需输入：
...
如果你想从列表的两端添加和弹出元素，你可以使用 deque（双端队列）。与只允许你从一端添加和删除元素的普通列表不同，deque 允许你从列表的左右两端添加和删除元素。如果有一个大的栈，这使得它非常方便。Deque 位于 *collections* 模块中。在下面的示例中，看看我们如何能够在列表的两端添加元素。
...
如果你有多个字典，并且想将它们包装成一个单元，该怎么办？你使用什么？Python 有一个名为 ChainMap 的类，它将不同的字典包装成一个单元。它将多个字典分组在一起以创建一个单元。ChainMap 来自 *collections* 模块。以下是它的工作原理：
...
当你执行循环（特别是非常大的循环）时，你可以显示一个智能的进度条。一个名为 **tqdm** 的库创建了一个进度计，用于跟踪循环的进度。要安装该库，请运行：

**pip install tqdm**

假设你有一个 100000 的范围，你希望你的循环遍历它，并且在每次循环后，你希望你的代码休眠 0.001 秒。以下是使用 **tqdm** 来实现的方法，以便你可以监控循环的进度。当你运行此代码时，你将获得下面的进度计作为输出。
...
你现在可以在进度计中看到 "progress" 这个词。你可以放入任何你想要的描述。进一步探索这个模块吧。
...
下面，我们使用 **pywhatkit** 将文本转换为手写体并将其保存为图像。然后我们使用 cv2 库来查看图像。运行下面的代码：
...
![](img/18cae09c3dc47caa2996fe5aa9c10ef1_90_0.png)
...
下面，我们使用 pyautogui 来截屏。然后我们保存图像，并使用 cv2 和 numpy 将其从 RGB 转换为 BGR。我们转换图像以便 cv2 库可以读取它。运行下面的代码看看效果。你可以使用以下命令安装其他库：
...
默认情况下，函数只返回一个值，然后就停止执行。下面，我们在 *return* 语句中有3个值。当我们调用该函数时，它会返回一个包含所有三个项目的元组。
...
如果我们想返回三个独立的值呢？在Python中如何实现？我们为每个要返回的值创建一个变量。这样就可以单独访问每个值。请看下面的代码：
...
Python使得下载YouTube视频变得超级简单。只需几行代码，你就可以将喜欢的视频保存到电脑上。下载视频所需的Python库是 **pytube**。使用以下命令安装：

```
pip install pytube
```

首先，我们将模块导入到脚本中。然后获取我们试图下载的视频链接。
...
在Python中，我们可以轻松地将字符串转换为字符串列表。我们可以将 *list()* 函数与 *map()* 函数结合使用。map函数返回一个map对象，它是一个迭代器。*map()* 函数接受两个参数：一个函数和一个可迭代对象。下面，列表是一个可迭代对象，*str()* 是函数。然后我们使用 *list()* 函数返回一个列表。*split()* 方法在空白字符处分割或拆分字符串。
...
如果你有两个序列，并且想同时遍历它们，该怎么办？在Python中如何实现？这时zip()函数就派上用场了。zip()函数将序列中的项目配对。序列一中索引为1的元素与序列二中索引为1的元素配对。这个过程会重复用于所有其他索引。让我们演示如何同时遍历两个序列。
...
从输出中你可以看到，我们使用zip()函数组合了两个列表。我们将名字和姓氏配对了。

## 83 | Extend 与 Append

Extend和append都是列表方法。它们都将元素添加到现有列表中。*append()* 方法将一个（1）项目添加到列表末尾，而 *extend()* 方法则不是。*extend()* 方法将多个项目添加到列表末尾。下面，我们使用 *append()* 方法将 *numbers2* 追加到 *numbers1*。你可以看到整个 *numbers2* 列表作为一个（1）项目被追加到了numbers1中。
...
当我们使用 *extend()* 方法时，请注意与上面 *append()* 方法的区别。*extend()* 方法获取 *numbers2* 中的元素，并将它们逐个追加到 *numbers1* 中，而不是作为一个整体列表。请看下面：
...
什么是 __slots__？Slots用于类中。它们基本上是实例对象将具有的属性。以下是slots的实际使用示例：
...
我们期望Cars类中的每个对象都具有这些属性——make和brand。

那么为什么要使用slots呢？我们使用slots是因为它们有助于节省内存空间。当我们使用slots时，类对象占用的空间更少。请看以下示例中对象大小的差异：
...
从上面的输出中你可以看到，使用slots节省了内存空间。

## 85 | 使用Python为图片添加水印

只需几行代码，我们就可以使用Python为图片添加水印。我们将使用pillow库来完成此任务。使用以下命令安装pillow模块：
...
![](img/18cae09c3dc47caa2996fe5aa9c10ef1_98_0.png)

现在让我们从pillow模块导入类，并编写为这张图片添加水印所需的代码。
...
![](img/18cae09c3dc47caa2996fe5aa9c10ef1_99_0.png)

## 86 | 解压Zip文件

Python有一个模块可以用来解压zip文件。这是一个内置模块，所以我们不需要安装它。如果文件与Python脚本在同一目录中，我们只需将文件名传递给代码。我们使用 *with* 语句打开文件。这将确保文件在操作结束时自动关闭。我们以读模式打开文件，因为我们只想读取文件的内容。*printdir()* 方法返回我们正在解压的文件的所有内容。*extractall()* 方法解压zip文件中的所有文件。
...
你可以使用Python生成假数据。Python有一个名为Faker的库，可以生成不同类型的数据。使用以下命令安装Faker：
...
你也可以使用此模型生成个人资料数据。下面，我们生成个人资料数据并将其传递给 pandas 以生成一个 Pandas DataFrame。
...
|---|---|---|---|---|---|---|
| 0 | hanson | Steven Kennedy | M | 8563 Jackson Crest Suite 324\nLawrenceand, CA... | ctumer@hotmail.com | 1964-08-17 |
| 1 | akane | Dara Martin | F | 025 Love Manor\nMeganstad, CA 83700 | iongblanca@gmail.com | 1961-11-07 |
| 2 | vmsbride | Ariel Greene | F | 3939 Cynthia Rapids Suite 704\nWest Dewiesberg... | youngjoe@hotmail.com | 1912-03-29 |
| 3 | stephannguyen | Rebecca Young | F | 71951 Zimmerman Overpass\nWest Sonya, FL 82366 | benjaminwilson@gmail.com | 1945-11-09 |
| 4 | freidrichard | Suzanne Walker | F | 073 Lopez Neck Suite 940\nPort Sarahshire, NY... | joseboyer@hotmail.com | 1975-07-14 |
| 5 | robert25 | Anthony James | M | 2301 Roth Motorway\nNew Debra, OH 35787 | gbelle@yahoo.com | 1907-09-04 |
| 6 | dennis64 | Crystal Stevenson | F | 344 Gray Loop Apt. 303\nLake Lindsay, IN 22871 | douglasarbara@yahoo.com | 1951-11-29 |
| 7 | christimcclhy | Violet Hanson | F | 8751 Ross Ways Apt. 016\nBrownville, NH 56161 | hendersonjames@gmail.com | 1932-03-18 |
| 8 | lrodriguez | Eric Rodriguez | M | 3267 John Loop Suite 346\nLake Sandraport, AL... | jacob48@yahoo.com | 1964-06-11 |
| 9 | amandacitrin | Paul Tanner | M | 237 Le Ferry Suite 142\nLake Carlhaven, MT 46444 | frenchmanuel@yahoo.com | 1996-08-23 |

使用此模块，你还可以生成许多其他类型的数据。请进一步探索该模块。

## 88 | 使用 more_itertools 展平列表

展平列表的方法有很多。我们已经介绍过其他方法。还有另一种方法可以展平嵌套列表。我们可以使用一个名为 **more_itertools** 的模块。我们将使用此模块中的 *collapse()* 类。这是一个单行解决方案。首先，使用以下命令安装该模块：
...
*collapse()* 方法的强大之处在于它可以展平任何列表，甚至是元组列表。请看下面：
...
下面，我们编写一个代码来计算给定数字的阶乘。一个数字的阶乘是从 1 到该数字的所有整数的乘积。3 的阶乘计算为 (1 * 2 * 3)，结果是 6。下面函数中的 *for 循环* 执行此计算。下面，我们计算整数 8 的阶乘。
...
下面是一个计算机器内存使用情况的脚本。这是我在当前使用的机器上运行此脚本时得到的结果。由于 **psutil** 以字节为单位给出结果，我们使用 (1024**3) 将它们转换为千兆字节。
...
在 Python 中，有两种连接字符串的方法。第一种是使用拼接（基本上是使用 + 运算符连接字符串）。第二种是使用字符串方法 join()。让我们找出哪种方法更快。我们将使用 time 模块中的 perf_counter 来计时代码的运行时间。
...
print(f'连接时间是 {end - start:.5f}')
输出：
连接时间是 0.00290
```

从输出中你可以看到，join() 方法比拼接更快。如果你想要更快的代码，请使用 join() 方法。但是，你应该注意，当你有更多字符串要连接时（至少超过 3 个字符串），join() 方法执行得更快。拼接方法执行较慢，因为每次拼接时，Python 都必须创建一个字符串并分配内存。这会减慢过程。但通过使用 `join()` 方法，Python 只创建一个字符串。这使得过程快得多。尝试运行代码并记录你的发现。请注意，你的执行时间将与我的输出不同。
...
1000 是 Python 中的默认递归限制。Python 设置此限制是为了防止堆栈溢出。但是，Python 也提供了一种方法来更改递归限制以适应你的需求。以下是你如何调整限制。
...
你可以使用 Python 获取关于一个国家的信息。Python 有一个名为 **countryinfo** 的模块，它返回关于国家的数据。你所要做的就是将国家名称传递给该模块。假设我们想了解赞比亚的省份。我们输入国家“Zambia”作为参数，然后使用 provinces 方法返回赞比亚的所有省份。
...
假设我们想获取关于一个给定国家所使用货币的信息；我们只需使用 currency 方法。下面使用 **currencies()** 方法来确定中国的货币。
...
我们之前已经看到如何使用函数计算一个数字的阶乘。你知道吗，你可以编写一个单行函数来计算一个数字的阶乘？为了实现这一点，我们需要使用 *itertools* 中的 reduce 函数和 *lambda* 函数。
...
你知道吗，你可以使用 Python 纠正文本中的拼写错误？Python 有一个名为 *textblob* 的库，可以纠正拼写错误。使用 pip 安装该库：

**pip install -U textblob**

下面，我们将一个包含拼写错误的字符串传递给 *textblob*。你可以看到错误已被更正为正确的拼写。Textblob 会为拼写错误的单词选择最可能的正确单词。
...
这个库的另一个有趣之处在于，你可以用它来将单词复数化。见下文：
...
给定一个列表，如何检查列表中的所有元素是否相同。我们可以使用 Python 的 *all()* 函数。这个函数接受一个可迭代对象作为参数，如果所有元素都为真，则返回 True。我们可以用它来检查列表中的所有元素是否相同。如果相同，它将返回 True。
...
如果你的文本中有想要屏蔽的脏话，你可以使用 Python 的 profanity 模块。使用 pip 安装：
...
默认情况下，它使用星号 (*) 来屏蔽脏话。但是，你也可以设置自己的屏蔽字符。见下面的示例。我们将 $ 设置为屏蔽字符。
...
如果你只想检查一个字符串是否包含脏话，你可以使用 *contains_profanity()* 方法，如果包含脏话，它将返回布尔值 True，如果不包含脏话，则返回 False。
...
你还可以创建一个自定义的要屏蔽的单词列表。如果文本包含你列表中的单词，那么它将被屏蔽。
...
如何检查一个数组是否是单调的？如果数组的元素按降序或升序排列，则该数组是单调的。例如，[4, 5, 5, 7] 是单调的。[4, 10, 2, 5] 不是单调的。我们使用 all() 函数来检查数组中的数字是升序还是降序。如果它们是升序或降序，它将返回 True。如果不是，它将返回 False。
...
一个数字的因数是所有可以整除给定数字而没有余数的数字。我们将使用 range 函数来获取所有用于除以给定数字的数字。如果其中任何一个数字没有返回余数，我们希望打印出这些数字。以下是完整的代码，使用 for 循环和 if 语句。
...
![](img/18cae09c3dc47caa2996fe5aa9c10ef1_119_0.png)

Gumroad 链接：[https://benjaminb.gumroad.com/l/zybjn](https://benjaminb.gumroad.com/l/zybjn)

Amazon 链接：[https://www.amazon.com/dp/B00TQ83JQB](https://www.amazon.com/dp/B00TQ83JQB)

2. Master Python Fundamentals: The Ultimate Guide for Beginners

![](img/18cae09c3dc47caa2996fe5aa9c10ef1_119_1.png)

Amazon 链接：[https://www.amazon.com/dp/B0BKVMC6DR](https://www.amazon.com/dp/B0BKVMC6DR)

封面图片

图片由 Freepik 的 liuzishan 提供
