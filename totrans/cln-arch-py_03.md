...
> 原文：[https://www.thedigitalcatbooks.com/pycabook-chapter-01/](https://www.thedigitalcatbooks.com/pycabook-chapter-01/)
...
In the rest of the book, we will design together part of a simple web application that provides a room renting system. So, let's consider that our "Rent-o-Matic" application^([[1](#fd-58102884)]) is running at [https://www.rentomatic.com](https://www.rentomatic.com), and that a user wants to see the available rooms. They open the browser and type the address, then clicking on menus and buttons they reach the page with the list of all the rooms that our company rents.

Let's assume that this URL is `/rooms?status=available`. When the user's browser accesses that URL, an HTTP request reaches our system, where there is a component that is waiting for HTTP connections. Let's call this component "web framework"^([[2](#fd-54651647)]).
...
![The web framework serving HTTP](../Images/a310516e1746da7b71d809116b6ca785.png)The web framework serving HTTP
...
![The business logic](../Images/afc423201f73500320f3bca14eafc350.png)The business logic
...
***![](../Images/07c9999c184018213f3bfc4b4c902b70.png)The storage***
...
***Inversion of control happens in two phases. First, the called object (the database in this case) is wrapped with a standard interface. This is a set of functionalities shared by every implementation of the target, and each interface translates the functionalities to calls to the specific language^([[3](#fd-33501696)]) of the wrapped implementation.***
...
***![The storage interface](../Images/85cec2f977b76c2397802b1486f0b7ba.png)The storage interface***
...
***![Inversion of control on the storage interface](../Images/aa24cf11fcf4a3e27515f07ca4bbc306.png)Inversion of control on the storage interface***
...
***![The business logic extracts data from the storage](../Images/a0de932ea207de9d0894271b2353b1c9.png)The business logic extracts data from the storage***
...
***![The business logic returns processed data to the web framework](../Images/412e495f5aa209edd764e1fa65824bcc.png)The business logic returns processed data to the web framework***
...
***![The web framework returns the data in an HTTP response](../Images/e4b6ce92027a26aa198e39c8d4afa4b5.png)The web framework returns the data in an HTTP response***
...
***![The web framework replaced by a CLI](../Images/179d70402b2999599ed5160872cb40ed.png)The web framework replaced by a CLI******![A database replaced by a more trivial file-based storage](../Images/063c5d5885cb494eb48ab604e8507c5d.png)A database replaced by a more trivial file-based storage***
...
***![Testing the web layer in isolation](../Images/d224037cf8f8a3df62c3dd6ed643fa3e.png)Testing the web layer in isolation******![Detailed setup of the web layer testing, width=80%](../Images/0bc5e22566f89d5f79ef5be8103fe6aa.png)Detailed setup of the web layer testing***
...
I was inspired by the Sludge-O-Matic™ from Day of the Tentacle*** ***[2](#fr-54651647)

There are many more layers that the HTTP request has to go through before reaching the actual web framework, for example the web server, but since the purpose of those layers is mostly to increase performances, I am not going to consider them until the end of the book.*** ***[3](#fr-33501696)
